...
import { FaArrowLeft, FaCommentDots, FaUserMd, FaSearch } from 'react-icons/fa';
import { useRouter } from 'next/navigation';
import MainLayout from '@/app/components/Layout/MainLayout';
import { useAuth } from '@/app/context/AuthContext';
import { listenToUserSessions, ChatSession } from '@/app/services/chatService';
...
export default function ChatsPage() {
  ...
  useEffect(() => {
    if (user) {
      const unsubscribe = listenToUserSessions(user.uid, (data) => {
        setSessions(data);
        setLoading(false);
      });
      return () => unsubscribe();
          ...
          {!selectedSession ? (
                ...
                {loading ? (
                  ...
                  filteredSessions.map((session) => (
                    ...
                    >
                      <div className="w-14 h-14 rounded-2xl bg-indigo-500 flex items-center justify-center text-white font-bold text-xl uppercase">
                        {session.doctorName?.[0]}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-1">
                          <h4 className="font-bold text-gray-900 dark:text-white truncate">{session.doctorName}</h4>
                  ...
                  <div className="text-center py-20">
                    ...
                    <button 
                      ...
                      className="mt-4 px-6 py-2 bg-indigo-500 text-white rounded-xl font-bold text-xs"
                    >
                      Browse Doctors
                    </button>
                  </div>
            ...
            >
              <ChatWindow
                ...
                receiverId={selectedSession.doctorId}
                receiverName={selectedSession.doctorName || 'Doctor'}
